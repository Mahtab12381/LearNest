...
class CartController {
  async add(req, res) {
    try {
      ...
      const extTransaction = await Transaction.find({
        user: learner_id,
      });
        if (extTransaction) {
            let matchedCourse = false;
            extTransaction.forEach((Transaction) => {
                if (Transaction.courses.includes(Cart.course)&&!Transaction.cancelled&&!Transaction.approved) {
                matchedCourse = true;
                }
            });
            if (matchedCourse) {
                ...
                res,
                HTTP_STATUS.BAD_REQUEST,
                "You have pending request for this course"
                );
            }
        }
      ...
      if (extCart) {
        const updatedList = await CartModel.findByIdAndUpdate(
          extCart._id,
          { $push: { courses: Cart.course } },
          { new: true }
        );
        if (!updatedList) {
      ...
      const updateList = await CartModel.findByIdAndUpdate(
        newCart._id,
        { $push: { courses: Cart.course } },
        { new: true }
      );
  ...
  async getMyCart(req, res) {
    try {
      const learner_id = req.user._id;
      const Cart = await CartModel.findOne({ learner: learner_id }).populate(
        "courses",
        "name _id rating thumbnail"
      );
      if (!Cart) {
      ...
      return response(res, HTTP_STATUS.OK, "Cart fetched successfully", Cart);
    } catch (e) {
      return response(res, HTTP_STATUS.INTERNAL_SERVER_ERROR, "Internal Error");
    }
  }
  ...
  async getAllCart(req, res) {
    try {
      const Carts = await CartModel.find().populate(
        "courses",
        "-reviews -contents -sections"
      );
      if (!Carts) {
        return response(res, HTTP_STATUS.NOT_FOUND, "No Item in Cart");
      }
      return response(res, HTTP_STATUS.OK, "Cart fetched successfully", Carts);
    } catch (e) {
      return response(res, HTTP_STATUS.INTERNAL_SERVER_ERROR, "Internal Error");
    }
  }
